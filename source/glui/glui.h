/****************************************************************************
  ...
  GLUI211 modifications 2001 Andre Bleau 
    To retrieve Paul Rademacher GLUI 2.10 original version, compile with
    NOGLUI211 defined.
...
#ifdef NOGLUI211	/* To suppress v2.11 extensions */
# ifdef GLUI211
#  undef GLUI211
# endif
#else
...
#ifdef GLUI211 /* MENUBUTTON */
#define GLUI_VERSION 2.11f    /********** Current version **********/
#else
...
#ifdef GLUI211 /* MENUBUTTON */
#define GLUI_PANEL_MENUBAR   3

/** Different menu button types **/
#define GLUI_MENUBUTTON_PLAIN 0
#define GLUI_MENUBUTTON_FLAT  1
#endif
...
#ifdef GLUI211 /* OPTION GLUI_DOUBLE */
#define  GLUI_DOUBLE             ((long)(1<< 6))
#endif
...
#ifdef GLUI211 /* friend class declaration */
...
/************************************************************/
/*                                                          */
/*               MenuButton class (container)               */
/*                                                          */
/************************************************************/

class GLUI_MenuButton : public GLUI_Button
{
private:
  int  glut_menu_id;
  int  tied_button;
public:
  int  data_type;
  void set_menu( int new_menu ) { glut_menu_id = new_menu; };
  int  get_menu( void ) { return glut_menu_id; };
  void set_tied_button( int new_tied_button ) {
          switch(new_tied_button) { 
             case GLUT_LEFT_BUTTON: 
             case GLUT_MIDDLE_BUTTON: 
             case GLUT_RIGHT_BUTTON:
                tied_button = new_tied_button;
          } 
       };
  int  get_tied_button( void ) { return tied_button; };
  int  mouse_down_handler( int local_x, int local_y ) { return false; };
  int  mouse_up_handler( int local_x, int local_y, int same ) { return false; };
  int  mouse_held_down_handler( int local_x, int local_y, int inside ) { return false; };
  ...
  GLUI_MenuButton( void ) {
    sprintf( name, "MenuButton: %p", this );
    type         = GLUI_CONTROL_MENUBUTTON;
    h            = GLUI_BUTTON_SIZE;
    w            = 75;
    alignment    = GLUI_ALIGN_CENTER;
    can_activate = true;
	 data_type    = GLUI_MENUBUTTON_FLAT,
    glut_menu_id = 0;
    tied_button  = GLUT_LEFT_BUTTON;
  };
};
...
#ifdef GLUI211 /* MENUBUTTON */
  GLUI_MenuButton  *add_menubutton( char *name, 
                 int data_type=GLUI_MENUBUTTON_FLAT,
                 int menu=0, int tied_button=GLUT_LEFT_BUTTON);
  GLUI_MenuButton  *add_menubutton_to_panel( GLUI_Panel *panel, char *name, 
                 int data_type=GLUI_MENUBUTTON_FLAT,
				     int menu=0, int tied_button=GLUT_LEFT_BUTTON);
  GLUI_Panel       *get_mainpanel(void) { return main_panel; };
#endif
...
#ifdef GLUI211 /* BUGFIX spinner name */
  void set_name( char *string ) { if (edittext) edittext->set_name(string); };
#endif
  void set_float_limits( float low,float high,int limit_type=GLUI_LIMIT_CLAMP);
  void set_int_limits( int low, int high,int limit_type=GLUI_LIMIT_CLAMP);
